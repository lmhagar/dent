...
DesignOneSample <- function(mu = NULL, sigma = NULL, deltaL = -Inf,
  ...
  if (length(targetPower) == 1){
    ...
    if (!is.finite(deltaU)){
      mid_val <- ((stats::qnorm(targetPower) + stats::qnorm(1 - alpha))*sigma/(mu - deltaL))^2
      upper_val <- ((stats::qnorm((3 +targetPower)/4) + stats::qnorm(1 - alpha))*sigma/(mu - deltaL))^2
    }
    else if (!is.finite(deltaL)){
      mid_val <- ((stats::qnorm(targetPower) + stats::qnorm(1 - alpha))*sigma/(deltaU - mu))^2
      upper_val <- ((stats::qnorm((3 +targetPower)/4) + stats::qnorm(1 - alpha))*sigma/(deltaU - mu))^2
    }
    else{
      ## find starting point for root-finding algorithm using normal approximations to
      ## the t-distribution
      a_cons <- (deltaU - mu)/sigma
      b_cons <- (deltaL - mu)/sigma
  ...
  if (length(n) == 1){
    ...
    NOTE <- "mu is not in (deltaL, deltaU), so we compute the type I error rate."
