...
#' @param sigma1 the anticipated standard deviation for intra-subject comparison in the first sequence (group 1). That is, \eqn{\sigma_1} is the standard deviation of all \eqn{y_{i11} - y_{i21}} observations, where \eqn{i} denotes the \eqn{i}th subject in the 1st sequence.
#' @param sigma2 the anticipated standard deviation for intra-subject comparison in the second sequence (group 2). That is, \eqn{\sigma_2} is the standard deviation of all \eqn{y_{i12} - y_{i22}} observations, where \eqn{i} denotes the \eqn{i}th subject in the 2nd sequence.
...
#'
#' @return The sample sizes or power estimate are returned as a list with supplementary information. If `targetPower` and `q` are specified to find sample sizes `n1` and `n2`, a plot of the approximated power curve will also appear in the plot pane if `plot = TRUE`.To find a sample size that corresponds to a different `targetPower`, save this function's output to an object and use the `UpdateTargetPower()` function.
#' @export
DesignCrossover2x2Unequal <- function(diff = NULL, sigma1 = NULL, sigma2 = NULL, deltaL = -Inf,
  ...
  if (length(targetPower) == 1 & length(q) == 1){
    ...
    results <- structure(list(n1 = n1_temp, n2 = n2_temp, q = q, diff = diff, sigma1 = 2*sigma1, sigma2 = 2*sigma2,
                              ...
                              design = "Crossover2x2Unequal", CI = CI), class = "power.en.test")
    if (sigma1 == sigma2){
...
DesignCrossoverDualUnequal <- function(diff = NULL, sigma1 = NULL, sigma2 = NULL, deltaL = -Inf,
  ...
  if (length(targetPower) == 1 & length(q) == 1){
    ...
    results <- structure(list(n1 = n1_temp, n2 = n2_temp, q = q, diff = diff, sigma1 = sqrt(8)*sigma1, sigma2 = sqrt(8)*sigma2,
                              ...
                              method = METHOD, upper.bound = ceiling(upper_c*upper_val),
                              samps = samps, seed = seed, sobol = sobol, copies = copies,
                              design = "CrossoverDualUnequal", CI = CI), class = "power.en.test")
    if (sigma1 == sigma2){
